...
public class skill7Use : MonoBehaviour {
        ...
        if (skillstate != -1f && skillstate != -2f)
        {
            if (mycdimg != null)
            {
                ...
                mycdimg.transform.GetComponent<RectTransform>().localPosition = new Vector3(mycdimg.transform.GetComponent<RectTransform>().localPosition.x,
                    -0.5f * imglengthy * (hero.skill7.coolDown - skillstate) / hero.skill7.coolDown, mycdimg.transform.GetComponent<RectTransform>().localPosition.z);
            }
        }
    }
